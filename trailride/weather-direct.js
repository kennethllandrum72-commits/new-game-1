(()=>{
 ...
 function score(temp,rain,hum,aqi,ozone=0){
  let s=10;
  // Heat is intentionally weighted heavily for outdoor exercise planning.
  if(temp>=105)s-=7;else if(temp>=100)s-=6;else if(temp>=95)s-=4.5;else if(temp>=90)s-=3;else if(temp>=85)s-=1.5;
  if(hum>=85)s-=1.5;else if(hum>=75)s-=.8;else if(hum>=65)s-=.3;
  // Extra combined heat/humidity penalty.
  if(temp>=95&&hum>=70)s-=1.5;else if(temp>=90&&hum>=75)s-=1;
  if(rain>=70)s-=3;else if(rain>=40)s-=1.5;else if(rain>=20)s-=.5;
  if(aqi>150)s-=3;else if(aqi>100)s-=2;else if(aqi>50)s-=.75;
  if(checked('respiratory')){
   if(aqi>100)s-=2;else if(aqi>50)s-=1;
   if(ozone>120)s-=1.5;
   if(temp>=95)s-=1.5;else if(temp>=90)s-=.75;
   if(hum>=80)s-=.75;
  }
  if(checked('heatSensitive')){if(temp>=100)s-=3;else if(temp>=95)s-=2;else if(temp>=90)s-=1;}
  if(checked('cardio')){if(temp>=100)s-=2;else if(temp>=95)s-=1.25;else if(temp>=90)s-=.5;}
 ...
 async function load(lat,lon){
  try{
   ...
   const hi=Math.round(wx.daily?.temperature_2m_max?.[di]??wx.current?.temperature_2m??0),lo=Math.round(wx.daily?.temperature_2m_min?.[di]??0),rain=Math.round(wx.daily?.precipitation_probability_max?.[di]??0);
   ...
   let rows=[];wx.hourly?.time?.forEach((t,i)=>{if(t.slice(0,10)!==day)return;let hr=+t.slice(11,13);if(hr<6||hr>20)return;let ai=aq?.hourly?.time?.indexOf(t)??-1,aqi=ai>=0?+(aq.hourly.us_aqi?.[ai]||0):0,ozone=ai>=0?+(aq.hourly.ozone?.[ai]||0):0,temp=+(wx.hourly.temperature_2m?.[i]||0),rp=+(wx.hourly.precipitation_probability?.[i]||0),hum=+(wx.hourly.relative_humidity_2m?.[i]||0);rows.push({t,temp,rp,hum,aqi,ozone,s:score(temp,rp,hum,aqi,ozone)})});
   ...
   $('overallScore').textContent=best?best.avg.toFixed(1):score(hi,rain,wx.current?.relative_humidity_2m||0,maxAqi||0,0).toFixed(1);
   window.__trailrideWeatherOK=true;
  }catch(e){$('weatherTitle').textContent='Live weather unavailable';$('weatherSummary').textContent='Tap Refresh to retry live weather.';$('bestWindow').textContent='Weather service did not respond. Tap Refresh to retry.';window.__trailrideWeatherOK=false;}
 }
 ...
 setTimeout(reload,600);$('refreshBtn')?.addEventListener('click',()=>setTimeout(reload,200));$('daySelect')?.addEventListener('change',reload);$('stateSelect')?.addEventListener('change',()=>setTimeout(reload,1800));$('locateBtn')?.addEventListener('click',()=>{navigator.geolocation?.getCurrentPosition(p=>load(p.coords.latitude,p.coords.longitude),()=>setTimeout(reload,400),{enableHighAccuracy:true,timeout:8000,maximumAge:300000})});document.querySelectorAll('.healthOpt').forEach(x=>x.addEventListener('change',reload));window.TrailRideWeather={reload,load};
})();